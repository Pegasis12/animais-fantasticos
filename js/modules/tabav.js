export default function initTabNav(){
  ...
  if(tabmenu.length && tabcontent.length){ //.leght pq se retornar o total de itens dentro maior que 0 é true, caso for 0 é false
    tabcontent[0].classList.add("ativo"); //mantendo a primeira section como ativo
  
    function activeTab(index){  //recebe o parametro de index, para identificar qual section recebe ativo apos sem clicado no tabmenu
      tabcontent.forEach((item) => { // limpando o ativo de outros itens ao da start na funcao
    ...
    tabmenu.forEach((item, index) => { //passando o parametro index tbm, pois ele leva esse mesmo index para o parametro de activetab
